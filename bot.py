...
def get_service_keyboard():
    ...
    for sid in live_ranges.keys():
        display = sid.upper()
        buttons.append([InlineKeyboardButton(display, callback_data=f"service_{sid}")])
...
async def text_handler(update: Update, context):
    if "GET NUMBER" in update.message.text.upper():
        await update.message.reply_text("👇 সার্ভিস সিলেক্ট করুন:", reply_markup=get_service_keyboard())
