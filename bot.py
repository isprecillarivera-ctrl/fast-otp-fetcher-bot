...
def get_country_keyboard():
    ...
    for code, data in ALLOWED_COUNTRIES.items():
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"range_{code}_{code}"  # range_{something}_{range}
        )])
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code != 200:
                logger.warning(f"API {endpoint} failed: {r.status_code} - {r.text[:200]}")
                return None
            return r.json()
    ...
    if query.data.startswith("range_") or query.data.startswith("chgnum_"):
        ...
        if res and res.get("meta", {}).get("status") == "ok":
            ...
            btn = [[InlineKeyboardButton(
                "🔄 Change Number", 
                callback_data=f"chgnum_{parts[1]}_{parts[2]}"
            )]]
...
async def text_handler(update: Update, context):
    ...
    if "GET NUMBER" in text:
        ...
        await update.message.reply_text(
            "📡 Live OTP দেখতে চ্যানেলে জয়েন করুন:",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📡 View Live OTP Channel", url=f"https://t.me/{OTP_CHANNEL.replace('@', '')}")
            ]])
        )
