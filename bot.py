...
      buttons = []  
        
      # এপিআই থেকে পাওয়া রেঞ্জগুলো অটোমেটিক আপডেট হবে  
...
              buttons.append([InlineKeyboardButton(f"🔷 {c_flag} {c_name} 
  ({r}) 🔷", callback_data=f"range_{service_name}_{r}")])  
        
      buttons.append([InlineKeyboardButton("🔙 Back", callback_data="back_to_services")])  

      await message_obj.reply_text(f"🔷 *Active Ranges for  
 {service_name.upper()}* 🔷", reply_markup=InlineKeyboardMarkup(buttons), 
...
 message_id=original_msg_id, text="❌ *OTP Timeout! Try another number.*", 
  parse_mode=ParseMode.MARKDOWN)  
...
              # ক্লিন নাম্বার ব্যবহার করা হয়েছে যাতে শুধু একটি '+' থাকে  
...
              asyncio.create_task(check_otp_loop(context,  
 query.message.chat_id, num_data.get("id"), sent_msg.message_id,  
 clean_num, c_flag, c_name, parts[1]))  
...
  lambda u, c: show_services_menu(u.message) if "GET NUMBER" in  
 u.message.text else None))  
      app.run_polling()  
